...
if [ ! -e $base/conf/nid ];then
	touch $base/conf/nid
	echo $NID > $base/conf/nid
fi
...
if [ -n OTTER_MANAGER_ADDRESS ];then
	sed -i "s/otter.manager.address = 127.0.0.1:1099/otter.manager.address = $OTTER_MANAGER_ADDRESS/g" conf/otter.properties
fi
